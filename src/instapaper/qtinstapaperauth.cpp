...
#include "qtinstapaperauth_p.h"
#include <QCryptographicHash>
#include <QDateTime>
#include <QStringList>
#include <QDebug>

QtInstapaperAuthPrivate::QtInstapaperAuthPrivate()
{
    qsrand(QTime::currentTime().msec());
    this->callbackUrl = QUrl("https://www.instapaper.com"); // Default callback url
}
...
QtInstapaperAuth::QtInstapaperAuth(QObject *parent) :
    QObject(parent),
    d_ptr(new QtInstapaperAuthPrivate)
{
}

QtInstapaperAuth::~QtInstapaperAuth()
{
    delete d_ptr;
}

// Setters
void QtInstapaperAuth::setType(QtInstapaperAuth::RequestType type)
{
    d_ptr->requestType = type;
}

void QtInstapaperAuth::setConsumerKey(const QString &consumerKey)
{
    d_ptr->consumerKey = consumerKey;
}

void QtInstapaperAuth::setConsumerSecret(const QString &consumerSecret)
{
    d_ptr->consumerSecret = consumerSecret;
}

void QtInstapaperAuth::setCallbackUrl(const QUrl &callbackUrl)
{
    d_ptr->callbackUrl = callbackUrl;
}

void QtInstapaperAuth::setToken(const QString &token)
{
    d_ptr->oauthToken = token;
}

void QtInstapaperAuth::setTokenSecret(const QString &tokenSecret)
{
    d_ptr->oauthTokenSecret = tokenSecret;
}

void QtInstapaperAuth::setVerifier(const QString &verifier)
{
#ifdef QT5
...
// Getters
QtInstapaperAuth::RequestType QtInstapaperAuth::type() const
{
    return d_ptr->requestType;
}

QString QtInstapaperAuth::token() const
{
    return d_ptr->oauthToken;
}

QString QtInstapaperAuth::tokenSecret() const
{
...
QByteArray QtInstapaperAuth::generateAuthHeader(const QUrl &requestUrl,
                                                 QtInstapaperAuth::HttpMethod httpMethod,
...
*/
QString QtInstapaperAuth::generateSignature(const QUrl &requestUrl,
                                             const QMultiMap<QString, QString> &requestParameters,
                                             QtInstapaperAuth::HttpMethod method) const
{
...
*/
QString QtInstapaperAuth::hmac_sha1(const QString& message, const QString& key) const
{
#ifdef QT5
