...
void QtReadabilityPrivate::executeRequest(const QUrl &requestUrl, const QString &httpMethod, const QtReadabilityParams &requestParams)
{
    ...
    if (httpMethod == "GET"){
//        qDebug() << "Requesting Server with Get Method";
        request.setRawHeader("Authorization", auth->generateAuthHeader(request.url()));
        request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
//        qDebug() << auth->generateAuthHeader(request.url());
        manager->get(request);
//        Q_FOREACH(QByteArray bite, request.rawHeaderList()){
//              qDebug() << bite.data() << request.rawHeader(bite);
//         }
    }else if (httpMethod == "POST"){
//        qDebug() << "Got into POST Reqest";
...
// Q_FOREACH(QByteArray bite, reply->rawHeaderList()){
//      qDebug() << bite.data() << reply->rawHeader(bite);
// }
...
//            qDebug() << "pair value:" <<pair;
        ...
        emit responseReceived(replyString,reply->url().toString(),qint32(reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt()));
